...
- 生成的 `Reactive.Action → Model → Observe → Screen + Widget` 完整链路。
...
3. 用方法组或生成的特性绑定，不在热路径创建捕获 lambda。
4. UI 状态在主线程修改，避免为响应容器增加锁。

## 冷路径

首次类型初始化、首次委托缓存、Screen/资源创建、容量扩张、异步等待源以及用户主动进行的字符串格式化允许分配。Screen、Reactive、Widget 和列表 Cell 均会池化，以避免重复打开时重新创建主要 UI 对象。
